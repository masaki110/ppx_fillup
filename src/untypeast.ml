...
let constructor_declaration sub cd =
  ...
  Type.constructor ~loc ~attrs ~vars:cd.cd_vars
...
let extension_constructor sub ext =
  ...
  Te.constructor ~loc ~attrs (map_loc sub ext.ext_name)
    ...
    | Text_decl (vs, args, ret) ->
        Pext_decl
          (vs, constructor_arguments sub args, Option.map (sub.typ sub) ret)
...
let expression sub exp =
  ...
  let desc =
    ...
    | Texp_instvar (_, path, name) ->
        Pexp_ident
          { loc = sub.location sub name.loc; txt = lident_of_path path }
  ...
  | _ ->
      let desc =
        ...
        | Tmod_constraint (mexpr, _, Tmodtype_explicit mtype, _) ->
            Pmod_constraint
              (sub.module_expr sub mexpr, sub.module_type sub mtype)
