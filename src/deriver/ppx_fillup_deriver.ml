...
let expr_of_string type_decl affix = evar @@ Ppx_deriving.mangle_type_decl affix type_decl
    ...
    | x::xs -> 
      ...
      | [%expr show] ->
        [Vb.mk 
          ...
          (polymorphize [%expr {pp=(fun x -> [%e apply_inner (Some "pp") @@ expr_of_string (`Prefix "pp")] x)}])]
        @check_plugin xs
      | [%expr eq] -> 
        [Vb.mk 
          (pat_of_string (`Prefix "equal")) 
          (polymorphize [%expr {equal=(fun x y -> [%e apply_inner None @@ expr_of_string (`Prefix "equal")] x y)}])]
        @check_plugin xs
      | [%expr ord] ->
        [Vb.mk 
          (pat_of_string (`Prefix "compare")) 
          (polymorphize [%expr {compare=(fun x y -> [%e apply_inner None @@ expr_of_string (`Prefix "compare")] x y)}])]
        @check_plugin xs
      | [%expr enum] ->
        [Vb.mk 
          (pat_of_string (`Suffix "to_enum")) 
          (polymorphize [%expr {to_enum=(fun x -> [%e apply_inner None @@ expr_of_string (`Suffix "to_enum")] x)}]);
         Vb.mk 
          (pat_of_string (`Suffix "of_enum")) 
          (polymorphize [%expr {of_enum=(fun x -> [%e apply_inner None @@ expr_of_string (`Suffix "of_enum")] x)}]);]
        @check_plugin xs
      | _ -> []
      end
    | [] -> []
...
let derivers_type_decl typ_decls =
  let attributes = List.concat (List.map (fun { ptype_attributes = attrs;_ } -> attrs) typ_decls) in
  ...
  let deriver_exprs =
    ...
    | Some (PStr [{ pstr_desc = Pstr_eval (
                    { pexp_desc = Pexp_tuple exprs;_ }, []);_ }]) ->
      exprs
    | Some (PStr [{ pstr_desc = Pstr_eval (
                    { pexp_desc = (Pexp_ident _ | Pexp_apply _);_ } as expr, []);_ }]) ->
      [expr]
