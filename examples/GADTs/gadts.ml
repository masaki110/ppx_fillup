...
and ('a, 'c, 'w) fcmp_k = { fcmp_k : 'b. ('b, 'c) fmt * ('a, 'b) fmt -> 'w }
        ...
        x#m_fcmp
          {
            fcmp_k =
              (fun (a, b) ->
                printer a (fun sa -> printer b (fun sb -> k (sa ^ sb))));
          }
...
let prefixp str inp =
  if
    String.length str <= String.length inp
    && str = String.sub inp 0 (String.length str)
  then String.sub inp (String.length str) (String.length inp - String.length str)
...
let intS inp =
  ...
  let rec loop acc i =
    if i >= n then (acc, "")
      ...
      else (acc, String.sub inp i (n - i))
  in
  if n = 0 then raise (Scan_error "int") else loop 0 0

(* val charS : string -> char * string *)
let charS inp =
  if String.length inp = 0 then raise (Scan_error "char")
  else (inp.[0], String.sub inp 1 (String.length inp - 1))

let rec scanner : 'a 'b. ('a, 'b) fmt -> string -> 'b -> 'a * string = function
  | FLit (eq, str) ->
      fun inp consumer -> (cast (symm eq) consumer, prefixp str inp)
  | FInt eq ->
      fun inp consumer ->
...
let e_lam f =
  Lam
    (object
       method m_lam : 'w. ('a, 'w) lam_k -> 'w = fun k -> k.lam_k (refl (), f)
    end)
...
let e_app f x =
  App
    (object
       method m_app : 'w. ('a, 'w) app_k -> 'w = fun k -> k.app_k (refl (), f, x)
    end)
...
let rec eval : 'a. 'a exp -> 'a = function
  | Int (eq, x) -> cast eq x
  | Lft (_, x) -> x
  | Inc eq -> cast eq succ
  | Lam x ->
      x#m_lam { lam_k = (fun (eq, f) -> cast eq (fun x -> eval (f (e_lft x)))) }
  | App x -> x#m_app { app_k = (fun (eq, f, x) -> cast eq ((eval f) (eval x))) }
